(ns synthego.main
  "Kotodama WASM entrypoint for the Synthego clean-room actor (L4) — Clojure port.
  ...
  facts (`synthego.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "synthego")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"synthego.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "synthego-compat" :tier tier :entities entities} 200])
...
(defn start! [] :synthego-compat/ready)
